...
# API KEY 
api_key = st.secrets["ow_api_key"]


# API CALL from the open weather webpage
...
# Function for fetching historical data
...
st.write('---')
st.subheader("Using Open Weather Map API ")
st.write('---')



city_name = st.sidebar.text_input("City Name", placeholder="Please enter your city")

if city_name:
        res, json = getweather(city_name)
        # st.write(res)
        st.success('Current Temperature: ' + str(round(res[1],2)))
        st.info('Humidity: ' + res[6])
